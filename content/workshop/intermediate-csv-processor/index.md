...
4. Select Camel Main 4.16.0 from the options that appear when clicking the Camel version. Other options present some variations related to the versions releases that may differ in some of the components presentation.
...
> [!TIP]
> Always click Save after configuring each component to preserve your changes.
...
1. **Hover** over the second component (log) in the route
...
6. Configure the following:
   ...
   | **header** **+** icon | `Id,BIRTHDATE,DEATHDATE,PREFIX,FIRST,LAST,SUFFIX,MAIDEN,MARITAL,RACE,ETHNICITY,GENDER,BIRTHPLACE,ADDRESS,CITY,STATE,COUNTY,ZIP,LAT,LON` | Defines CSV column names |
...
1. **Hover** over the last component in the route
2. Click the **Replace** icon
...
1. In the Kaoto extension panel, click the **New** icon next to **Integrations**
...
1. Add a **Multicast** processor after the filter
...
1. In VSCode, create a new file in the `patient-csv` directory
2. Name it `application.properties`
...
1. **Get a compose file:**
   - Visit the [Kaoto examples repository](https://github.com/KaotoIO/kaoto-examples)
...
Before creating the route, you need a Postgres instance.
...
2. **Get a compose file:**
   - Visit the Kaoto examples repository and download the [compose file](https://github.com/KaotoIO/kaoto-examples/blob/main/csv-processor/kafka-postgres.compose.yaml)
...
```properties
# Kafka and Postgres Configuration
csv-processor.kafka.brokers = localhost:9092
...
1. In the Kaoto extension, click **New** next to **Integrations**
...
1. Add a **PostgreSQL Sink** Kamelet (search for "postgresql sink")
2. In the **All** tab, configure:
...
#### Step 4: Add Custom Logger

1. Add a **Log** processor (note: this is different from the Log component)
2. In the **All** tab, configure:
